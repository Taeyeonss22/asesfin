...
import { Users, RefreshCw, Plus, DollarSign, Printer, Search, Calendar } from 'lucide-react';
import CreditForm from '../components/CreditForm';
import PaymentForm from '../components/PaymentForm';
import CalendarioModal from '../components/CalendarioModal';

export default function CreditosGrupales({ session }) {
    ...
    <div className="animate-fade-in">
      <div className="flex justify-between items-center mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 style={{ margin: 0 }}>Créditos Grupales</h1>
        </div>
        <div className="flex gap-4">
          ...
          <button className="btn btn-primary" onClick={() => setShowCreditForm(true)}>
            <Plus size={16} /> Nuevo Crédito Grupal
          </button>
        </div>
      </div>
        ...
        <div className="flex items-center gap-2 mb-4" style={{ maxWidth: '400px' }}>
          <div className="form-group w-full" style={{ marginBottom: 0, position: 'relative' }}>
            ...
            <input 
              ...
              placeholder="Buscar por nombre del grupo o folio..." 
              ...
              {credits.filter(item => 
                ...
                <tr>
                  <td colSpan="8" className="text-center text-muted" style={{ padding: '3rem' }}>
                    {searchTerm ? 'No se encontraron créditos.' : 'No hay créditos grupales registrados.'}
                  </td>
                </tr>
              ) : (
                credits.filter(item => 
                  ...
                  <tr key={item.credito_id}>
                    <td className="font-medium">GRP-{item.credito_id.split('-')[0].toUpperCase()}</td>
                    <td>{item.nombre_cliente || `Grupo Solidario ${item.credito_id.split('-')[0]}`}</td>
